...
import { AccountFacade } from 'src/app/feature/account/account.facade';
import { IUser } from 'src/app/core/auth/model/user.interface';

@Component({
    selector: "app-employess-absences",
    templateUrl: "./employess-absences-worker.component.html",
    styleUrls: ["./employess-absences-worker.component.sass"]
})
export class EmployessAbsencesWorkerComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.user = this._employeeFacade.getUserData();
    ...
    public openAddAbsence(): void {
        ...
        this.workersListSubscription = this._employeeFacade.getWorkersList$().subscribe((workersList: IWorkerAccountInfromation[]) => {
            if (workersList) {
                const worker: IWorkerAccountInfromation[] = [];
                worker.push(
                    workersList.find((element: IWorkerAccountInfromation) => {
                        return element.uuid === this.user.userUuid;
                    })
                );
                dialogRef.componentInstance.workersList = worker;
            }
        });
